...
// GenerationResult reports what generateM3U8ForPlaylist actually did,
...
// CountEntries returns how many track entries the M3U8 file at path
...
func CountEntries(path string) (count int, ok bool) {
...
// WriteToPlaylistsDir writes baseName.m3u8 into <root>/Playlists.
//
...
func WriteToPlaylistsDir(
	...
	if guardShrink {
		...
		if existingCount, ok := CountEntries(m3u8Path); ok &&
	...
	if err := WriteFile(baseName, playlistDir, paths, jellyfinPath, root); err != nil {
		return result, err
	}
	result.Written = true
	return result, nil
...
func WriteFile(m3u8Name string, outputDir string, filePaths []string, jellyfinMusicPath string, musicRoot string) error {
	if len(filePaths) == 0 {
		return nil
	}
