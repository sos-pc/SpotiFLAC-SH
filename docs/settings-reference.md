...
> **`SPOTIFY_ID` is always embedded.** Every successful download carries a `SPOTIFY_ID` tag (Vorbis comment for FLAC, `TXXX:SPOTIFY_ID` for MP3, custom `----:com.apple.iTunes:SPOTIFY_ID` atom for M4A). This is not user-configurable. The watcher uses it to regenerate M3U8 playlists from the filesystem (`meta.BuildSpotifyIDIndex`).
