...
All endpoints are mounted under `/api/v1/`. Every request (except `/auth/login` and the SPA static routes) requires authentication via a JWT Bearer token or an API key.
...
Generic multipart upload. Saves the file to `<TMPDIR>/spotiflac_upload_<basename>` and returns `{ "path": "..." }`. Used by other handlers that need a temp file. Path traversal is blocked with `filepath.Base()`. Authentication: same as v1 (Bearer token or API key).
