...
| [override-rework-plan.md](override-rework-plan.md) | 🧭 | ⏸️ décidé, non commencé | **2026-07-17**, investigation faite | Refonte de la sélection de service vers le modèle UI (`auto`+chaîne honorée / explicite = forcer). **4 défauts** (override + 3 trouvés en creusant : auto mono-piste cassé par l'async, `serviceURL` biaisé Tidal, collapse auto→qobuz). Sous-questions ouvertes en §7. Préalable au portage S6. |
