...
|---|---|---|---|
| [settings-source-of-truth.md](settings-source-of-truth.md) | 🔍+🧭 | **étapes 1, 2, 3 (backend + frontend) + watchlists ✅ vérifiées prod** ; reste 4 (cover/paroles), 5 (M3U8), 6 (`getSettings`/défauts) | Les réglages n'avaient pas de source unique (3 stockages, 2 mécanismes). **Décidé : backend autoritaire**, un seul override (`service`). §7 = carte exhaustive + plan phasé, §7.5 = avancement. |
