...
| [settings-source-of-truth.md](settings-source-of-truth.md) | 🔍+🧭 | 🔴 **problème ouvert** | **2026-07-17**, code + observation prod | Les réglages n'ont **pas de source unique** : 3 stockages (serveur/localStorage/cache) + 2 mécanismes de livraison (batch envoie / mono-piste rempli par le serveur) → deux downloads peuvent diverger. Décision de modèle à trancher (§4-5). |
