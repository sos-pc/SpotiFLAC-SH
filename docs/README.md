...
| [override-rework-plan.md](override-rework-plan.md) | 🧭 | **phases 1, 2a ✅ vérifiées ; 2b codée** (vérif navigateur en attente) ; **3 soldée sans code** (défauts déjà alignés, libellé déjà exact, Deezer à ne PAS réactiver — mort, mesuré le 07-18) | Refonte de la sélection de service vers le modèle UI (`auto`+chaîne honorée / explicite = forcer). La boucle de fallback **existait déjà** (`ExecuteDownload`) — réutilisée, pas recodée. |
