...
| [override-rework-plan.md](override-rework-plan.md) | 🧭 | ✅ décidé, phase 1 prête | **2026-07-17**, investigation faite | Refonte de la sélection de service vers le modèle UI (`auto`+chaîne honorée / explicite = forcer). **4 défauts** (override + auto mono-piste cassé par l'async, `serviceURL` biaisé Tidal, collapse auto→qobuz). **La boucle de fallback existe déjà (`ExecuteDownload`) — réutilisée, pas recodée.** §7 tranché. Préalable au portage S6. |
